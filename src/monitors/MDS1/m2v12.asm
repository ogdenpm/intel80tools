$       TITLE   ('INTELLEC SERIES I/II MONITOR, VERSION 1.2, 4 JANUARY 1978')
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
;*                                                                      *
;*                  INTELLEC SERIES I/II BOOT/MONITOR      9800605A     *
;*                          VERSION 1.2                                 *
;*                                                                      *
...
DATE    EQU     0401H   ; CREATION DATE, 4 JANUARY 1978
; NOTE:
...
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
BBASE    SET     0F800H         ; BASE ADDRESS OF MONITOR

;        MODULE  0F800H,0FFFFH

        ORG     BBASE           ; TOP 2K OF 64K ADDRESS SPACE
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
;
...
CI0:
        ...
        PUSH    H               ; SAVE HL
        LHLD    MEMTOP
        MVI     L,ILOC-1 AND 0FFH; HL NOW POINTS TO CONFIGURATION BYTE STORED
...
CRTOUT:
        PUSH    H               ; SAVE H,L
        LHLD    MEMTOP
        MVI     L,ILOC-1 AND 0FFH; HL NOW POINTS TO CONFIGURATION BYTE IN EXIT TEMPLATE 
...
LO:                             ; LIST OUTPUT
        ...
        MVI     A,LILOC AND 0FFH; A := LSB OF LILOC ADDRESS
...
CS0:
        ...
        PUSH    H               ; SAVE H,L
        LHLD    MEMTOP
        MVI     L,ILOC-1 AND 0FFH; HL POINTS TO CONFIGURATION BYTE IN EXIT TEMPLATE
...
CS3:
        ...
        MVI     A,CSLOC AND 0FFH; CONSOLE = USER DEFINED LOCAL CONSOLE, BRANCH
...
RESTART:
        ...
        PUSH    H               ; DUMMY PUSH TO RESERVE SPACE IN STACK FOR
                                ...
                                ;   BYTE
        LHLD    MEMTOP
        MVI     L,ILOC-1 AND 0FFH; HL NOW POINTS TO CONFIGURATION BYTE IN
        ...
        LHLD    MEMTOP
        MVI     L,EXIT AND 0FFH ; HL NOW POINTS TO EXIT CODE AT TOP OF RAM
...
;       ---------------------------------------------------------------------
        ...
        LHLD    MEMTOP
        MVI     L,TLOC AND 0FFH ; HL NOW POINTS TO TLOC IN TOP PAGE OF RAM
...
RST1:                           ; PROGRAMMED RESTART AT A BREAKPOINT (TRAP)
        ...
        LHLD    MEMTOP
        MVI     L,LLOC AND 0FFH ; HL NOW POINTS TO LLOC IN EXIT CODE IN TOP OF RAM
        MOV     M,E             ; USER'S L VALUE PRIOR TO RESTART IS STORED IN LLOC
        INX     H
        MOV     M,D             ; USER'S H VALUE PRIOR TO RESTART IS STORED IN HLOC
;       ---------------------------------------------------------------------
        MVI     L,PLOC-1 AND 0FFH; HL POINTS TO LSB OF JMP INSTR IN EXIT CODE
...
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
        ORG     0FFFDH
MNCKSM: DB      06DH            ; CHKSUM MONITOR TO 01EH
        DB      00              ; UNUSED BYTE
