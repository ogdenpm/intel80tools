...
void FatlIO(file_t *fileP, word errNum)
{
	...
	PrintStr(errHeader, sizeof(errHeader) - 1);
	PrintStr(fileP->sNam, 6);
	PrintStr(aName, sizeof(aName) - 1);	
	PrintStr(fileP->fNam, 15);
	PrintStr(aError, sizeof(aError) - 1);
	...
	PrintStr(aTerminate, sizeof(aTerminate) - 1);
	//if (debugFlag )
	//	REBOOTVECTOR();
	//else
		Exit();
