...
* **ASM80X** - set to the asmx.pl program. *Experimental*.
...
* **locate-overlaps** - this is the same as locate with the exception that overlaps names are not treated as an error. *Depreciated*
     ...
     It was designed to support using obj2bin by including junk data to synthesise what was in memory when Intel originally built isis.bin.  A newer cleaner option is to use a patch file with the obj2bin command line.
...
Only five implicit build rules are defined, one to generate a hex file from an asm48 source file, the rest generate .obj files from .plm, .asm, .f and .asmx files. The rules are
...
* **clean::** - used to clean *.obj, *.abs, *.lst, *.lin, *.map, *.hex, *.bin files.
  ...
  Note as toolbox can contains a utility clean, the clean target noted here is only generated when clean, distclean or rebuild are specified on the command line. Although this prevents just building the toolbox's clean application, in practice this isn't a problem as it is not usually built separately.
...
### Part of tex makefile
    ...
    all:: $(TARGETS)					~~ note $(MAKE) not needed as no auto extract
