...
static word ParseIsisName(char *cleanIsisName, const char *isisPath)
{
	...
	for (int i = 0; i < sizeof(deviceMap) / sizeof(deviceMap[0]); i++) {
		if (strncmp(cleanIsisName + 1, deviceMap[i].dev, 2) == 0)
			if (deviceMap[i].devtype != 3)
				return isalnum(*isisPath) ? ERROR_BADFILENAME : ERROR_SUCCESS;
			else {
				int j;
				char *s = &cleanIsisName[4];
				for (j = 0; j < 6 && isalnum(*isisPath); j++)
					*s++ = tolower(*isisPath++);
				*s = 0;
				if (j == 0 || isalnum(*isisPath))
					return ERROR_BADFILENAME;
				if (*isisPath == '.') {
					*s++ = *isisPath++;
					for (j = 0; j < 3 && isalnum(*isisPath); j++)
						*s++ = tolower(*isisPath++);
					*s = 0;
					if (j == 0 || isalnum(*isisPath))
						return ERROR_BADEXT;
	...
	*connP = -1;		// unused conn
	...
	if (strcmp(path, ":BB:") == 0) {
		*connP = handle;
		...
		return;
	} else if (path[0] == ':' ) {	// all other non file devices are not available
		*statusP = ERROR_NOTREADY;
		return;
	}		
...
//int isis_spath(const char *isisname, ISIS_STAT *result)
{
	...
	memset(infoP, 0, sizeof(spath_t));
	infoP->deviceId = 0xff;			// assume bad device

	if ((*statusP = ParseIsisName(isisName, pathP)) != ERROR_SUCCESS)
		return;

	for (deviceId = 0; deviceId < sizeof(deviceMap) / sizeof(deviceMap[0]); deviceId++)
		if (strncmp(isisName + 1, deviceMap[deviceId].dev, 2) == 0)
			break;
	// devciceId will be valid as invalid ones eliminated in ParseIsisName
	infoP->deviceId = deviceId;
	if ((infoP->deviceType = deviceMap[deviceId].devtype) == 3) { // a file
		sprintf(buf, "ISIS_F%c", isisName[2]);	// check it exists
		infoP->driveType = (isisName[2] == 0 || getenv(buf)) ? 4 : 0;
		char *s, *t;
		for (s = isisName + 4, t = infoP->name; *s; s++)
			if (*s == '.')
				t = infoP->ext;
			else
				*t++ = toupper(*s);
